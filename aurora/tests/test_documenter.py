...
async def test_documentation_sweep_queries_engine_and_saves_audience_blocks(mock_minion_task, mock_exists, mock_file):
    """
    FIXED: Patched run_minion_task directly instead of query_groq_llm. This prevents the crawler from hitting live APIs or generating error strings, allowing it to append records directly to report['processed_files'].
    """
    ...
    # REFACTOR: Remapped mock credential vectors out to use the new Gemini environment variables
    with patch.dict('os.environ', {'GEMINI_API_KEY': 'gen-lang-client-mock-crawler-key'}), patch('sys.stdout.write'):
