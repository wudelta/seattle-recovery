...
class HeadlessSessionAutomationTest(TestCase):
    ...
    def test_unauthenticated_request_throws_login_redirect(self):
        print("\n🧪 [TEST 1] Verifying security guardrails block unauthenticated callers...")
        response = self.client.post(self.target_url, data=json.dumps({"brief_content": "Sample Test"}), content_type="application/json")
        # Django's login_required decorator returns a 302 redirect status code
        self.assertEqual(response.status_code, 302)
        print("✅ [TEST 1] Security barrier confirmed. Request successfully redirected to login matrix.")
