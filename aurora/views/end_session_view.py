...
# Consolidated Core Graph Logic & Cloud Backup Imports
from core_logic.memory import summarize_session
from core_logic.sessions import end_session
from core_logic.backup import backup_and_stream_databases

# Setup structured file and terminal logger
logger = logging.getLogger("aurora.sessions.close")
logger.setLevel(logging.DEBUG)

# Absolute path tracking matrix for emergency local copies
LOCAL_BACKUP_DIR = os.path.join(os.getcwd(), 'core_logic/staging/backups')

def asynchronous_git_pipeline(commit_msg, logger_instance, user_id):
    """
    Runs the storage-conscious dual compressed database backup to Google Drive,
    followed by automated staging, committing, and pushing operations to GitHub.
    All executing on an isolated background thread to prevent Django server freezes.
    """
    print("\n🧵 [BACKGROUND THREAD] Asynchronous automation pipeline worker activated.")
    import subprocess

    # --- STEP 1: EXECUTE HEADLESS STORAGE-SAVER CLOUD BACKUP ---
    try:
        backup_result = backup_and_stream_databases(user_id)
        print(f"📡 [DRIVE WORKER] Rolling backup transaction status: {backup_result}")
    except Exception as drive_err:
        print(f"❌ [DRIVE WORKER FAILURE] Automated rolling database backup crashed: {str(drive_err)}")
        logger_instance.error(f"Google Drive automation hook crash: {str(drive_err)}")

    # --- STEP 2: RUN AUTOMATED GIT REPOSITORY SOURCE PROTECTION PUSH ---
    try:
        ...
        # Step B: Check if there are actual code changes staged before executing the commit
        ...
        if status_check.returncode == 1:
            ...
            print("ℹ️ [GIT WORKER] No local modifications detected since session initialization pass.")
            logger_instance.info("Git pipeline finalized: Zero new repository snapshot changes to sync.")
            
    except subprocess.CalledProcessError as git_err:
        err_output = git_err.stderr.decode('utf-8').strip() if git_err.stderr else str(git_err)
    ...
    print("🧵 [BACKGROUND THREAD] Worker thread operations finalized smoothly.\n")
...
def end_session_view(request):
    ...
    if not request.user.is_authenticated:
        ...
        return JsonResponse({'status': 'error', 'success': False, 'error': 'Unauthorized profile entry.'}, status=401)
    ...
    # --- 3. DISPATCH OFF-THREAD BACKGROUND PIPELINE ---
    print("🔍 [STAGE 3] Spawning safe background thread for Cloud Backup & Git automation pipeline...")
    ...
    git_thread = threading.Thread(
        ...
        args=(commit_msg, logger, user_id),
        daemon=True
    )
    git_thread.start()
    print("✅ [STAGE 3] Background thread detached. Moving cleanly to transaction finalization.")

    # --- 4. OPTIONAL LOCAL VALIDATION TRACE METRICS ---
    local_metrics = {"tracked": False, "note": "No synchronous emergency copies found."}
    if os.path.exists(LOCAL_BACKUP_DIR):
        files = [os.path.join(LOCAL_BACKUP_DIR, f) for f in os.listdir(LOCAL_BACKUP_DIR) if f.endswith('.zip')]
        if files:
            latest_file = max(files, key=os.path.getmtime)
            bytes_sz = os.path.getsize(latest_file)
            local_metrics = {
                "tracked": True,
                "latest_emergency_archive": os.path.basename(latest_file),
                "size_bytes": bytes_sz,
                "size_kb": round(bytes_sz / 1024, 2)
            }

    # --- 5. FORMULATE PURE DATA JSON CONTRACT PACKAGES ---
    ...
    return JsonResponse({
        ...
        'local_storage_audit': local_metrics,
        'git_pipeline_status': 'Asynchronous rolling cloud backup and repository pushes detached safely.'
    })
