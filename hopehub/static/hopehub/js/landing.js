// ======================================================================
// FILE: hopehub/static/hopehub/js/landing.js
// START: HOPEHUB_LANDING_GRAPH
// ======================================================================
document.addEventListener("DOMContentLoaded", function () {
    console.log("🪐 [Aurora Matrix] Initializing clean graph layout script...");

    const nodes = new vis.DataSet([
        {
            id: 1,
            label: "Hopehub Core",
            url: "/hopehub/",
            color: "#38bdf8",
            size: 30,
        },
        {
            id: 2,
            label: "About",
            url: "/hopehub/about_hopehub/",
            color: "#818cf8",
        },
        {
            id: 3,
            label: "Contact",
            url: "/hopehub/contact_us/",
            color: "#f97316",
        },
        {
            id: 5,
            label: "Resources",
            url: "/hopehub/",
            color: "#f43f5e",
        },
        {
            id: 6,
            label: "Account",
            url: "/hopehub/my_account/",
            color: "#d946ef",
        },
        {
            id: 9,
            label: "Mission",
            url: "/hopehub/mission_statement/",
            color: "#94a3b8",
        },
    ]);

    const edges = new vis.DataSet([
        { from: 1, to: 2 },
        { from: 1, to: 3 },
        { from: 1, to: 5 },
        { from: 1, to: 6 },
        { from: 1, to: 9 },
    ]);

    const container = document.getElementById("network-container");

    if (!container) {
        console.error(
            "❌ [Aurora Matrix ERROR] Core canvas anchor targeting ID " +
            "'#network-container' was not found."
        );
        return;
    }

    const data = {
        nodes: nodes,
        edges: edges,
    };

    const options = {
        nodes: {
            shape: "dot",
            font: {
                color: "#212529",
                size: 16,
            },
            chosen: {
                node: function (values, id, selected, hovering) {
                    if (hovering) {
                        values.size = values.size + 5;
                        values.borderColor = "#212529";
                        values.borderWidth = 2;
                    }
                },
                label: function (values, id, selected, hovering) {
                    if (hovering) {
                        values.color = "#000000";
                        values.mod = "bold";
                    }
                },
            },
        },
        edges: {
            color: "#94a3b8",
            width: 2,
        },
        interaction: {
            hover: true,
            zoomView: true,
        },
    };

    const network = new vis.Network(container, data, options);

    const MIN_ZOOM = 0.5;
    const MAX_ZOOM = 2.5;
    let isAdjusting = false;

    network.on("zoom", function () {
        if (isAdjusting) {
            return;
        }

        const currentScale = network.getScale();
        let targetScale = currentScale;
        let needsFix = false;

        if (currentScale < MIN_ZOOM) {
            targetScale = MIN_ZOOM;
            needsFix = true;
        } else if (currentScale > MAX_ZOOM) {
            targetScale = MAX_ZOOM;
            needsFix = true;
        }

        isAdjusting = true;

        if (needsFix) {
            network.moveTo({
                scale: targetScale,
                position: { x: 0, y: 0 },
                animation: false,
            });
        } else {
            network.moveTo({
                position: { x: 0, y: 0 },
                animation: false,
            });
        }

        isAdjusting = false;
    });

    network.on("hoverNode", function () {
        container.style.cursor = "pointer";
    });

    network.on("blurNode", function () {
        container.style.cursor = "default";
    });

    network.on("click", function (params) {
        if (params.nodes.length === 0) {
            return;
        }

        const nodeId = params.nodes[0];
        const clickedNode = nodes.get(nodeId);

        if (!clickedNode || !clickedNode.url) {
            return;
        }

        if (clickedNode.target === "_blank") {
            window.open(
                clickedNode.url,
                "_blank",
                "noopener,noreferrer"
            );
            return;
        }

        window.location.href = clickedNode.url;
    });

    console.log(
        "✅ [Aurora Matrix] Visual clusters initialization finalized smoothly."
    );
});
// ======================================================================
// FILE: hopehub/static/hopehub/js/landing.js
// END: HOPEHUB_LANDING_GRAPH
// ======================================================================