# ======================================================================
# FILE: hopehub/tests/test_page_mission_statement_hopehub.py
...
class HopehubMissionStatementViewProductionTest(TestCase):
    def setUp(self):
        self.test_user = User.objects.create_user(username="test_dev", password="password")
        self.expected_path = "templates/hopehub/mission_statement.html"
        try:
            db.cypher_query("MATCH (n:ComponentNode) WHERE n.file_path = '" + self.expected_path + "' DETACH DELETE n")
        except Exception:
            pass
        register_new_component(self.expected_path, "mission_statement_layout", "public", self.test_user, "COMPILER_MODULE", "Verification baseline")
# ======================================================================
...
# ======================================================================
    def test_forged_component_integrity(self):
        disk_path = os.path.join(os.getcwd(), "hopehub", "templates", "hopehub", "mission_statement.html")
        self.assertTrue(os.path.exists(disk_path), f"Layout canvas missing from disk path: {disk_path}")

        url = reverse("hopehub:mission_statement")
